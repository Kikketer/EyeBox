...
class PhotoButtonTester:
    def __init__(self):
        ...
        self.led_delay = 12.0  # 12 seconds
    ...
    def setup_gpio(self):
        ...
        try:
            ...
            lgpio.gpio_claim_input(
                self.gpio_handle, GOOD_PHOTO_BUTTON_PIN, lgpio.SET_PULL_UP
            )
            lgpio.gpio_claim_input(
                self.gpio_handle, BAD_PHOTO_BUTTON_PIN, lgpio.SET_PULL_UP
            )
            ...
            print(
                f"✓ GPIO initialized: Button on GPIO{GOOD_PHOTO_BUTTON_PIN} (physical pin 13)"
            )
            print(
                f"✓ GPIO initialized: Button on GPIO{BAD_PHOTO_BUTTON_PIN} (physical pin 15)"
            )
            print(
                f"✓ GPIO initialized: Good LED on GPIO{GOOD_LED_PIN} (physical pin 12)"
            )
    ...
    def setup_cameras(self):
        ...
        try:
            ...
            print(
                f"✓ Camera 1 initialized: {int(self.camera1.get(cv2.CAP_PROP_FRAME_WIDTH))}x{int(self.camera1.get(cv2.CAP_PROP_FRAME_HEIGHT))}"
            )
    ...
    def set_led(self, led_pin, state, led_name):
        ...
        try:
            if self.gpio_handle is not None:
                ...
                print(
                    f"✓ {led_name.capitalize()} LED {'ON' if state else 'OFF'} (3.3V, ~2mA)"
                )
        except Exception as e:
    ...
    def update_led_control(self, current_time):
        ...
        if (good_button_pressed or bad_button_pressed) and (
            self.good_led_on or self.bad_led_on
        ):
    ...
    def take_photo(self, button_type: str):
            ...
            if self.current_frame1 is not None:
                ...
                if button_type == "good":
                    response = requests.post(
                        UPLOAD_URL + "?type=complimentary", files=files, timeout=5
                    )
                elif button_type == "bad":
                    response = requests.post(
                        UPLOAD_URL + "?type=insult", files=files, timeout=5
                    )
        ...
        if success_count > 0:
            self.photo_count += 1
            print(
                f"✓ Photo set #{self.photo_count} complete ({success_count}/2 cameras)"
            )
            return True
        else:
    ...
    def run(self):
        ...
        try:
            ...
            while True:
                ...
                if (good_button_is_pressed or bad_button_is_pressed) and not (
                    good_button_was_pressed or bad_button_was_pressed
                ):
                    # Button just pressed
                    ...
                    if time_since_last > DEBOUNCE_TIME:
                        ...
                        print(
                            f"  ⏳ Debounced (wait {DEBOUNCE_TIME - time_since_last:.1f}s)"
                        )
